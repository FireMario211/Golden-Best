...
class $modify(PlayLayer) {
    ...
    void updateProgressbar() {
        /*char a[offsetof(GJBaseGameLayer, m_isDebugDrawEnabled) - (sizeof(CCLayer) + sizeof(TriggerEffectDelegate))];
        a = 1;
        char b[offsetof(GJBaseGameLayer, m_isPracticeMode) - (sizeof(CCLayer) + sizeof(TriggerEffectDelegate))];
        b = 1;*/
        /*char a[offsetof(PlayLayer, m_progressBar) - (sizeof(GJBaseGameLayer) + sizeof(CCCircleWaveDelegate) + sizeof(CurrencyRewardDelegate) + sizeof(DialogDelegate))];
        a = 1;
        char b[offsetof(PlayLayer, m_progressBar)];
        b = 1;*/
        PlayLayer::updateProgressbar();
        if (!m_fields->initialized && m_fields->percentLabel == nullptr) {
            // TODO: find members so I can use m_percentageLabel on Android 
            //
            ...
            if (dontChangeFont) {
                m_fields->goldenLabel = CCLabelBMFont::create("0%", "goldFont.fnt");
                m_fields->goldenLabel->setScale(m_fields->percentLabel->getScale() + 0.15F);
                m_fields->goldenLabel->setVisible(false);
                m_fields->goldenLabel->setAnchorPoint({0.5,m_fields->percentLabel->getAnchorPoint().y-0.05F});
                m_fields->goldenLabel->setZOrder(16);
                m_fields->percentLabel->getParent()->addChild(m_fields->goldenLabel);
            }
            m_fields->oldScale = m_fields->percentLabel->getScale();
            m_fields->initialized = true;
            */
            return;
        }
